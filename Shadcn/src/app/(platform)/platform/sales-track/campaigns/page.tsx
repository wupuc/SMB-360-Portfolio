...
import {
  demoCampaigns, OWNER_NAMES,
  type CampaignType, type CampaignStatus,
} from "@/lib/demo/sales-data"
import { isDemoMode } from "@/lib/demo/data"
import { fetchCampaigns } from "@/lib/supabase/salestrack"
...
type LocalCampaign = typeof demoCampaigns[number]
...
function campaignToForm(c: LocalCampaign): FormState {
  return {
    name:        c.name,
    type:        c.type,
...
export default function CampaignsPage() {
  ...
  const [campaigns, setCampaigns] = useState<LocalCampaign[]>(isDemoMode ? demoCampaigns : [])

  useEffect(() => {
    if (isDemoMode) return
    fetchCampaigns().then(data => setCampaigns(data as LocalCampaign[]))
  }, [])
  ...
  // Edit dialog
  const [editTarget, setEditTarget] = useState<LocalCampaign | null>(null)
  ...
  // ── Edit ─────────────────────────────────────────────────────────────────
  function openEdit(c: LocalCampaign) {
    setEditTarget(c)
  ...
  // ── Delete ────────────────────────────────────────────────────────────────
  function handleDelete(c: LocalCampaign) {
    ...
    return (
      <div className="grid gap-3 py-2">
        ...
        <div className="space-y-1">
          ...
          <Select
            value={values.type}
            onValueChange={v => onChange({ type: v as CampaignType })}
          >
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="email_blast">E-mail</SelectItem>
              <SelectItem value="event">Event</SelectItem>
              <SelectItem value="cold_outreach">Cold Outreach</SelectItem>
              <SelectItem value="linkedin">LinkedIn</SelectItem>
              <SelectItem value="trade_show">Targi</SelectItem>
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <Select
          value={statusFilter}
          onValueChange={v => setStatusFilter(v as "all" | CampaignStatus)}
        >
              ...
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between gap-2">
                  <CardTitle className="text-base leading-snug">
                    <Link
                      href={`/platform/sales-track/campaigns/${c.id}`}
              ...
              <CardContent className="flex-1 flex flex-col gap-3 text-sm">
                ...
                <div className="mt-auto pt-2">
                  <Button asChild size="sm" variant="outline" className="w-full h-9">
                    <Link href={`/platform/sales-track/campaigns/${c.id}`}>
                      Szczegóły
                    </Link>
                  </Button>
                </div>
