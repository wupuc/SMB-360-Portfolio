...
import { fetchClients, fetchOwners } from "@/lib/supabase/salestrack"
    ...
    <div className="grid gap-4 py-2">
      ...
      <div className="grid gap-1.5">
        ...
        <StarRating
          ...
          onChange={(v) => setForm((f) => ({ ...f, leadScore: v }))}
        />
      </div>
      {/* Przypisany */}
      <div className="grid gap-1.5">
...
export default function ClientsPage() {
  ...
  const [clients, setClients] = useState<Client[]>(isDemoMode ? (demoClients as Client[]) : [])
      ...
      prev.map((c) =>
        c.id === editingClient.id
          ? {
              ...
              leadScore: editForm.leadScore,
              website: editForm.website.trim(),
              phone: editForm.phone.trim(),
              notes: editForm.notes.trim(),
              address: editForm.address.trim(),
              nip: editForm.nip.trim(),
              regon: editForm.regon.trim(),
            }
          : c
      )
    )
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <Input
          placeholder="Szukaj po nazwie, mieście, branży…"
          value={search}
          ...
          >
            <CardContent className="pt-4 pb-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <span
                  className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium shrink-0 ${TYPE_CLASS[c.type]}`}
                >
                  {TYPE_LABELS[c.type]}
                </span>
              </div>
                ...
                {filtered.map((c) => (
                  <TableRow key={c.id}>
                    <TableCell className="font-medium">
                      <Link
                        href={`/platform/sales-track/clients/${c.id}`}
                    ...
                    <TableCell>
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${TYPE_CLASS[c.type]}`}
                      >
                        {TYPE_LABELS[c.type]}
                      </span>
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            router.push(`/platform/sales-track/clients/${c.id}`)
                          }
                        >
